...
def try_upgrade_lock(lock_file, read_acquired, upgrade_attempted, upgrade_result) -> None:
    lock = ReadWriteLock(lock_file)
    try:
        with lock.read_lock():
            read_acquired.set()
...
def try_downgrade_lock(lock_file, write_acquired, downgrade_attempted, downgrade_result) -> None:
    lock = ReadWriteLock(lock_file)
    try:
        with lock.write_lock():
            write_acquired.set()
