...
def acquire_read_lock(lock_file, acquired_event, release_event=None, 
                      timeout=-1, blocking=True, ready_event=None):
...
def acquire_write_lock(lock_file, acquired_event, release_event=None, 
                       timeout=-1, blocking=True, ready_event=None):
    if ready_event:
        ready_event.wait(timeout=10)
    ...
    try:
        ...
        print(f"Write lock process error: {e}")
        return False


# Helper function to try upgrading a read lock to a write lock (should fail)
def try_upgrade_lock(lock_file, read_acquired_event, upgrade_attempted_event, upgrade_result):
    ...
    try:
        ...
        print(f"Upgrade lock process error: {e}")
        upgrade_result.value = 4


# Helper function to try downgrading a write lock to a read lock (should fail)
def try_downgrade_lock(lock_file, write_acquired_event, downgrade_attempted_event, downgrade_result):
    lock = ReadWriteLock(lock_file)
    try:
        with lock.write_lock():
        ...
        print(f"Downgrade lock process error: {e}")
...
def cleanup_processes(processes):
    ...
    try:
        ...
        # Check for errors before terminating processes
        try:
            while True:
...
@pytest.mark.timeout(20)
def test_write_lock_excludes_other_write_locks(lock_file):
    ...
    p1 = mp.Process(target=acquire_write_lock, 
    ...
    p2 = mp.Process(target=acquire_write_lock, 
    ...
    with cleanup_processes([p1]):
        ...
        p3 = mp.Process(target=acquire_write_lock, 
                       args=(lock_file, write2_acquired, None))
...
@pytest.mark.timeout(20)
def test_write_lock_excludes_read_locks(lock_file):
    ...
    p2 = mp.Process(target=acquire_read_lock, 
...
@pytest.mark.timeout(20)
def test_read_lock_excludes_write_locks(lock_file):
    ...
    p1 = mp.Process(target=acquire_read_lock, 
                   args=(lock_file, read_acquired, release_read))
...
def chain_reader(idx, lock_file, release_count,
                 forward_wait, backward_wait, forward_set, backward_set):
    ...
    try:
        ...
        print(f"Chain reader {idx} error: {e}")

@pytest.mark.timeout(40)
def test_write_non_starvation(lock_file):
    ...
    for i in range(NUM_READERS):
        ...
        reader = mp.Process(
            target=chain_reader,
            args=(i, lock_file, release_count, chain_forward[i], chain_backward[i],
...
def try_upgrade_lock(lock_file, read_acquired, upgrade_attempted, upgrade_result):
    lock = ReadWriteLock(lock_file)
    try:
        with lock.read_lock():
            read_acquired.set()
            upgrade_attempted.set()
            try:
                with lock.write_lock():
                    pass  # Should not reach here
            except RuntimeError:
                upgrade_result.value = 0  # Expected behavior
            except Exception as e:
                print(f"Unexpected error during upgrade: {e}")
                upgrade_result.value = 2
    except Exception as e:
        print(f"Unexpected error acquiring read lock: {e}")
        upgrade_result.value = 1


def try_downgrade_lock(lock_file, write_acquired, downgrade_attempted, downgrade_result):
    lock = ReadWriteLock(lock_file)
    try:
        with lock.write_lock():
            write_acquired.set()
            downgrade_attempted.set()
            try:
                with lock.read_lock():
                    pass  # Should not reach here
            except RuntimeError:
                downgrade_result.value = 0  # Expected behavior
            except Exception as e:
                print(f"Unexpected error during downgrade: {e}")
                downgrade_result.value = 2
    except Exception as e:
        print(f"Unexpected error acquiring write lock: {e}")
        downgrade_result.value = 1


def recursive_read_lock(lock_file, success_flag):
    lock = ReadWriteLock(lock_file)
    try:
        with lock.read_lock():
            # First acquisition
        ...
        print(f"Recursive read lock error: {e}")
        success_flag.value = 0


@pytest.mark.timeout(10)
def test_recursive_read_lock_acquisition(lock_file):
...
@pytest.mark.timeout(10)
def test_lock_downgrade_prohibited(lock_file):
    ...
    p = mp.Process(target=try_downgrade_lock, 
                  args=(lock_file, write_acquired, downgrade_attempted, downgrade_result))
...
@pytest.mark.timeout(10)
def test_timeout_behavior(lock_file):
    ...
    with cleanup_processes([p1, p2]):
        ...
        # Make the timing constraints more lenient
...
@pytest.mark.timeout(10)
def test_non_blocking_behavior(lock_file):
...
# Move this function to module level (before the test functions)
def recursive_write_lock(lock_file, success_flag):
    lock = ReadWriteLock(lock_file)
    try:
        with lock.write_lock():
            # First acquisition
        ...
        print(f"Recursive write lock error: {e}")
        success_flag.value = 0


@pytest.mark.timeout(10)
def test_recursive_write_lock_acquisition(lock_file):
...
def acquire_write_lock_and_crash(lock_file, acquired_event):
    lock = ReadWriteLock(lock_file)
    with lock.write_lock():
        acquired_event.set()
...
def test_write_lock_release_on_process_termination(lock_file):
...
def acquire_read_lock_and_crash(lock_file, acquired_event):
    lock = ReadWriteLock(lock_file)
    with lock.read_lock():
        acquired_event.set()
        # Simulate process crash with infinite loop
...
@pytest.mark.timeout(15)
def test_read_lock_release_on_process_termination(lock_file):
...
@pytest.mark.timeout(15)
def test_single_read_lock_acquire_release(lock_file):
...
@pytest.mark.timeout(15)
def test_single_write_lock_acquire_release(lock_file):
...
@pytest.mark.timeout(15)
def test_read_then_write_lock(lock_file):
...
@pytest.mark.timeout(15)
def test_write_then_read_lock(lock_file):
...
if __name__ == "__main__":
    # Set up multiprocessing to spawn instead of fork
